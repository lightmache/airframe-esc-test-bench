...
/* [Global] */
partToGenerate = "ShellOnly"; //[ShellOnly,FuselageRear,FuselageNose,FuselageHatch,RootWing,TipWing,Fin,Elevon,ServoGuard]
...
HatchLen = 0.4*(rootChord+noseLen);
...
difference()
   ...
   #translate([noseCutX,0.1*bodyH,0.8*bodyWidth/2])
    ...
    #translate([noseCutX,trayFloorY-1.5*fuseJoinerD,0])
      rotate([0,0,0])
         rotate([0,90,0])
            FuseJoiner();  //cylinder(d=fuseJoinerD,h=fuseJoinerLen);
  if(fingerGrips)
